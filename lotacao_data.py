"""
Dados de Lotação Paradigma - TJPR
Atualizado em: 13/02/2026 01:53:41
Fonte: Mapeamento de data.py (ANEXO_II) com dados de lotação preservados

Regras de mapeamento:
  - Códigos baseados em data.py (ANEXO_II do Edital 01/2026)
  - Dados de lotação preservados quando unidade encontrada por nome
  - Unidades não encontradas: lotacao_real=5, lotacao_paradigma=5, status=EQUILIBRADA

Estatísticas:
  - Total de unidades: 606
  - Mapeadas com dados preservados: 458
  - Com valores padrão (5/5): 148
"""

# Mapeamento: Código Anexo II -> Dados de Lotação
LOTACAO_POR_CODIGO = {
    "A2-001": {
        "comarca": "Almirante Tamandaré",
        "unidade": "1ª VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-002": {
        "comarca": "Almirante Tamandaré",
        "unidade": "2ª VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-003": {
        "comarca": "Almirante Tamandaré",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-004": {
        "comarca": "Almirante Tamandaré",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 6,
        "lotacao_paradigma": 6,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-005": {
        "comarca": "Almirante Tamandaré",
        "unidade": "SECRETARIA ESPECIALIZADA DE MOVIMENTAÇÃO PROCESSUAL DAS VARAS CÍVEIS E DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-006": {
        "comarca": "Almirante Tamandaré",
        "unidade": "VARA DE FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-007": {
        "comarca": "Alto Paraná",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-008": {
        "comarca": "Alto Paraná",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 2,
        "lotacao_paradigma": 2,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-009": {
        "comarca": "Alto Piquiri",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-010": {
        "comarca": "Alto Piquiri",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-011": {
        "comarca": "Altônia",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-012": {
        "comarca": "Altônia",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-013": {
        "comarca": "Ampére",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 7,
        "diferenca": -2,
        "status": "DEFICITÁRIA",
    },
    "A2-014": {
        "comarca": "Ampére",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-015": {
        "comarca": "Andirá",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 4,
        "lotacao_paradigma": 4,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-016": {
        "comarca": "Andirá",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-017": {
        "comarca": "Antonina",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-018": {
        "comarca": "Antonina",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 3,
        "lotacao_paradigma": 5,
        "diferenca": -2,
        "status": "DEFICITÁRIA",
    },
    "A2-019": {
        "comarca": "Antonina",
        "unidade": "VARA CÍVEL, DA FAZENDA PÚBLICA, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL, JUIZADO ESPECIAL CÍVEL E JUIZADO ESPECIAL DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-020": {
        "comarca": "Apucarana",
        "unidade": "1ª VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-021": {
        "comarca": "Apucarana",
        "unidade": "2ª VARA CRIMINAL",
        "lotacao_real": 6,
        "lotacao_paradigma": 5,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-022": {
        "comarca": "Apucarana",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 9,
        "lotacao_paradigma": 8,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-023": {
        "comarca": "Apucarana",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 10,
        "lotacao_paradigma": 10,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-024": {
        "comarca": "Apucarana",
        "unidade": "VARA DE FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 8,
        "lotacao_paradigma": 7,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-025": {
        "comarca": "Arapongas",
        "unidade": "1ª VARA CRIMINAL",
        "lotacao_real": 0,
        "lotacao_paradigma": 5,
        "diferenca": -5,
        "status": "DEFICITÁRIA",
    },
    "A2-026": {
        "comarca": "Arapongas",
        "unidade": "2ª VARA CRIMINAL",
        "lotacao_real": 0,
        "lotacao_paradigma": 6,
        "diferenca": -6,
        "status": "DEFICITÁRIA",
    },
    "A2-027": {
        "comarca": "Arapongas",
        "unidade": "2ª VARA CÍVEL E DA FAZENDA PÚBLICA",
        "lotacao_real": 0,
        "lotacao_paradigma": 5,
        "diferenca": -5,
        "status": "DEFICITÁRIA",
    },
    "A2-028": {
        "comarca": "Arapongas",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 0,
        "lotacao_paradigma": 9,
        "diferenca": -9,
        "status": "DEFICITÁRIA",
    },
    "A2-029": {
        "comarca": "Arapongas",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 10,
        "lotacao_paradigma": 10,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-030": {
        "comarca": "Arapongas",
        "unidade": "VARA DE FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 0,
        "lotacao_paradigma": 6,
        "diferenca": -6,
        "status": "DEFICITÁRIA",
    },
    "A2-031": {
        "comarca": "Arapoti",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-032": {
        "comarca": "Arapoti",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 4,
        "lotacao_paradigma": 4,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-033": {
        "comarca": "Araucária",
        "unidade": "2ª VARA CÍVEL E DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-034": {
        "comarca": "Araucária",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-035": {
        "comarca": "Araucária",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 9,
        "lotacao_paradigma": 9,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-036": {
        "comarca": "Araucária",
        "unidade": "VARA CRIMINAL",
        "lotacao_real": 9,
        "lotacao_paradigma": 9,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-037": {
        "comarca": "Araucária",
        "unidade": "VARA DE FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 6,
        "lotacao_paradigma": 8,
        "diferenca": -2,
        "status": "DEFICITÁRIA",
    },
    "A2-038": {
        "comarca": "Assaí",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 4,
        "lotacao_paradigma": 4,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-039": {
        "comarca": "Assaí",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-040": {
        "comarca": "Assis Chateaubriand",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-041": {
        "comarca": "Assis Chateaubriand",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-042": {
        "comarca": "Astorga",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-043": {
        "comarca": "Astorga",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-044": {
        "comarca": "Bandeirantes",
        "unidade": "2ª VARA CÍVEL E DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-045": {
        "comarca": "Bandeirantes",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 4,
        "lotacao_paradigma": 4,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-046": {
        "comarca": "Bandeirantes",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-047": {
        "comarca": "Barbosa Ferraz",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-048": {
        "comarca": "Barbosa Ferraz",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-049": {
        "comarca": "Barracão",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-050": {
        "comarca": "Barracão",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-051": {
        "comarca": "Bela Vista do Paraíso",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-052": {
        "comarca": "Bela Vista do Paraíso",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 4,
        "lotacao_paradigma": 4,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-053": {
        "comarca": "Bocaiúva do Sul",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-054": {
        "comarca": "Bocaiúva do Sul",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-055": {
        "comarca": "Cambará",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-056": {
        "comarca": "Cambará",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-057": {
        "comarca": "Cambé",
        "unidade": "1ª VARA CÍVEL E DA FAZENDA PÚBLICA",
        "lotacao_real": 1,
        "lotacao_paradigma": 8,
        "diferenca": -7,
        "status": "DEFICITÁRIA",
    },
    "A2-058": {
        "comarca": "Cambé",
        "unidade": "2ª VARA CÍVEL E DA FAZENDA PÚBLICA",
        "lotacao_real": 6,
        "lotacao_paradigma": 6,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-059": {
        "comarca": "Cambé",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-060": {
        "comarca": "Cambé",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 6,
        "lotacao_paradigma": 6,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-061": {
        "comarca": "Cambé",
        "unidade": "VARA CRIMINAL",
        "lotacao_real": 6,
        "lotacao_paradigma": 5,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-062": {
        "comarca": "Cambé",
        "unidade": "VARA DE FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 6,
        "lotacao_paradigma": 5,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-063": {
        "comarca": "Campina da Lagoa",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-064": {
        "comarca": "Campina da Lagoa",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-065": {
        "comarca": "Campina Grande do Sul",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-066": {
        "comarca": "Campina Grande do Sul",
        "unidade": "VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-067": {
        "comarca": "Campina Grande do Sul",
        "unidade": "VARA CÍVEL E DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-068": {
        "comarca": "Campina Grande do Sul",
        "unidade": "VARA DE FAMÍLIA E SUCESSÕES, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL, JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA E DA INFÂNCIA E JUVENTUDE",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-069": {
        "comarca": "Campo Largo",
        "unidade": "1ª VARA CÍVEL E DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-070": {
        "comarca": "Campo Largo",
        "unidade": "2ª VARA CÍVEL E DA FAZENDA PÚBLICA",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-071": {
        "comarca": "Campo Largo",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 7,
        "diferenca": -2,
        "status": "DEFICITÁRIA",
    },
    "A2-072": {
        "comarca": "Campo Largo",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 8,
        "lotacao_paradigma": 8,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-073": {
        "comarca": "Campo Largo",
        "unidade": "VARA CRIMINAL",
        "lotacao_real": 6,
        "lotacao_paradigma": 8,
        "diferenca": -2,
        "status": "DEFICITÁRIA",
    },
    "A2-074": {
        "comarca": "Campo Largo",
        "unidade": "VARA DE FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 6,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-075": {
        "comarca": "Campo Mourão",
        "unidade": "1ª VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-076": {
        "comarca": "Campo Mourão",
        "unidade": "2ª VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-077": {
        "comarca": "Campo Mourão",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-078": {
        "comarca": "Campo Mourão",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 8,
        "lotacao_paradigma": 8,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-079": {
        "comarca": "Campo Mourão",
        "unidade": "VARA DE FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 6,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-080": {
        "comarca": "Cantagalo",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 7,
        "lotacao_paradigma": 6,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-081": {
        "comarca": "Cantagalo",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-082": {
        "comarca": "Capanema",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-083": {
        "comarca": "Capanema",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-084": {
        "comarca": "Capitão Leônidas Marques",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 6,
        "lotacao_paradigma": 7,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-085": {
        "comarca": "Capitão Leônidas Marques",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-086": {
        "comarca": "Carlópolis",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-087": {
        "comarca": "Carlópolis",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-088": {
        "comarca": "Cascavel",
        "unidade": "1ª VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-089": {
        "comarca": "Cascavel",
        "unidade": "1ª VARA DE FAMÍLIA E SUCESSÕES, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 7,
        "lotacao_paradigma": 7,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-090": {
        "comarca": "Cascavel",
        "unidade": "1º JUIZADO DE VIOLÊNCIA DOMÉSTICA E FAMILIAR CONTRA A MULHER, VARA DE CRIMES CONTRA CRIANÇAS, ADOLESCENTES E IDOSOS",
        "lotacao_real": 8,
        "lotacao_paradigma": 8,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-091": {
        "comarca": "Cascavel",
        "unidade": "1º JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 7,
        "diferenca": -2,
        "status": "DEFICITÁRIA",
    },
    "A2-092": {
        "comarca": "Cascavel",
        "unidade": "2ª VARA DE FAMÍLIA E SUCESSÕES E ACIDENTES DO TRABALHO",
        "lotacao_real": 7,
        "lotacao_paradigma": 7,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-093": {
        "comarca": "Cascavel",
        "unidade": "2º JUIZADO DE VIOLÊNCIA DOMÉSTICA E FAMILIAR CONTRA A MULHER, VARA DE CRIMES CONTRA CRIANÇAS, ADOLESCENTES E IDOSOS",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-094": {
        "comarca": "Cascavel",
        "unidade": "2º JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 6,
        "lotacao_paradigma": 7,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-095": {
        "comarca": "Cascavel",
        "unidade": "3ª VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-096": {
        "comarca": "Cascavel",
        "unidade": "3º JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 6,
        "lotacao_paradigma": 7,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-097": {
        "comarca": "Cascavel",
        "unidade": "4ª VARA CRIMINAL",
        "lotacao_real": 7,
        "lotacao_paradigma": 5,
        "diferenca": 2,
        "status": "SUPERAVITÁRIA",
    },
    "A2-098": {
        "comarca": "Cascavel",
        "unidade": "4ª VARA CÍVEL E EMPRESARIAL REGIONAL",
        "lotacao_real": 7,
        "lotacao_paradigma": 7,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-099": {
        "comarca": "Cascavel",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 28,
        "lotacao_paradigma": 28,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-100": {
        "comarca": "Cascavel",
        "unidade": "5ª VARA CÍVEL",
        "lotacao_real": 7,
        "lotacao_paradigma": 7,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-101": {
        "comarca": "Cascavel",
        "unidade": "VARA DA FAZENDA PÚBLICA",
        "lotacao_real": 8,
        "lotacao_paradigma": 5,
        "diferenca": 3,
        "status": "SUPERAVITÁRIA",
    },
    "A2-102": {
        "comarca": "Cascavel",
        "unidade": "VARA DA INFÂNCIA E JUVENTUDE",
        "lotacao_real": 8,
        "lotacao_paradigma": 11,
        "diferenca": -3,
        "status": "DEFICITÁRIA",
    },
    "A2-103": {
        "comarca": "Cascavel",
        "unidade": "VARA DE EXECUÇÕES PENAIS E CORREGEDORIA DOS PRESÍDIOS",
        "lotacao_real": 7,
        "lotacao_paradigma": 6,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-104": {
        "comarca": "Castro",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-105": {
        "comarca": "Castro",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 8,
        "lotacao_paradigma": 8,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-106": {
        "comarca": "Castro",
        "unidade": "VARA CRIMINAL",
        "lotacao_real": 1,
        "lotacao_paradigma": 7,
        "diferenca": -6,
        "status": "DEFICITÁRIA",
    },
    "A2-107": {
        "comarca": "Castro",
        "unidade": "VARA DE FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-108": {
        "comarca": "Catanduvas",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 6,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-109": {
        "comarca": "Catanduvas",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 4,
        "lotacao_paradigma": 4,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-110": {
        "comarca": "Centenário do Sul",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-111": {
        "comarca": "Centenário do Sul",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-112": {
        "comarca": "Cerro Azul",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-113": {
        "comarca": "Cerro Azul",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-114": {
        "comarca": "Chopinzinho",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-115": {
        "comarca": "Chopinzinho",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-116": {
        "comarca": "Cianorte",
        "unidade": "2ª VARA CÍVEL E DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-117": {
        "comarca": "Cianorte",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 9,
        "lotacao_paradigma": 7,
        "diferenca": 2,
        "status": "SUPERAVITÁRIA",
    },
    "A2-118": {
        "comarca": "Cianorte",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 8,
        "lotacao_paradigma": 8,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-119": {
        "comarca": "Cianorte",
        "unidade": "VARA CRIMINAL",
        "lotacao_real": 7,
        "lotacao_paradigma": 8,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-120": {
        "comarca": "Cianorte",
        "unidade": "VARA DE FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 4,
        "lotacao_paradigma": 6,
        "diferenca": -2,
        "status": "DEFICITÁRIA",
    },
    "A2-121": {
        "comarca": "Cidade Gaúcha",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-122": {
        "comarca": "Cidade Gaúcha",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-123": {
        "comarca": "Clevelândia",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-124": {
        "comarca": "Clevelândia",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 2,
        "lotacao_paradigma": 2,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-125": {
        "comarca": "Colombo",
        "unidade": "1ª VARA CRIMINAL",
        "lotacao_real": 4,
        "lotacao_paradigma": 6,
        "diferenca": -2,
        "status": "DEFICITÁRIA",
    },
    "A2-126": {
        "comarca": "Colombo",
        "unidade": "2ª VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 7,
        "diferenca": -2,
        "status": "DEFICITÁRIA",
    },
    "A2-127": {
        "comarca": "Colombo",
        "unidade": "2ª VARA CÍVEL",
        "lotacao_real": 6,
        "lotacao_paradigma": 7,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-128": {
        "comarca": "Colombo",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 6,
        "lotacao_paradigma": 6,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-129": {
        "comarca": "Colombo",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 13,
        "lotacao_paradigma": 13,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-130": {
        "comarca": "Colombo",
        "unidade": "VARA DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 6,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-131": {
        "comarca": "Colombo",
        "unidade": "VARA DA INFÂNCIA E JUVENTUDE, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-132": {
        "comarca": "Colombo",
        "unidade": "VARA DE FAMÍLIA E SUCESSÕES",
        "lotacao_real": 5,
        "lotacao_paradigma": 8,
        "diferenca": -3,
        "status": "DEFICITÁRIA",
    },
    "A2-133": {
        "comarca": "Colorado",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 4,
        "lotacao_paradigma": 4,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-134": {
        "comarca": "Colorado",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-135": {
        "comarca": "Congonhinhas",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-136": {
        "comarca": "Congonhinhas",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-137": {
        "comarca": "Corbélia",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-138": {
        "comarca": "Corbélia",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-139": {
        "comarca": "Cornélia Procópio",
        "unidade": "2ª VARA CÍVEL E DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-140": {
        "comarca": "Cornélia Procópio",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-141": {
        "comarca": "Cornélia Procópio",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-142": {
        "comarca": "Cornélia Procópio",
        "unidade": "VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-143": {
        "comarca": "Cornélia Procópio",
        "unidade": "VARA DE FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-144": {
        "comarca": "Coronel Vivida",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-145": {
        "comarca": "Coronel Vivida",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-146": {
        "comarca": "Cruzeiro do Oeste",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 6,
        "lotacao_paradigma": 6,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-147": {
        "comarca": "Cruzeiro do Oeste",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 8,
        "lotacao_paradigma": 7,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-148": {
        "comarca": "Cruzeiro do Oeste",
        "unidade": "VARA DE EXECUÇÕES PENAIS E CORREGEDORIA DOS PRESÍDIOS",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-149": {
        "comarca": "Curitiba",
        "unidade": "10ª VARA CRIMINAL",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-150": {
        "comarca": "Curitiba",
        "unidade": "10ª VARA CÍVEL",
        "lotacao_real": 9,
        "lotacao_paradigma": 8,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-151": {
        "comarca": "Curitiba",
        "unidade": "11º JUIZADO ESPECIAL CÍVEL E CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-152": {
        "comarca": "Curitiba",
        "unidade": "12ª VARA CRIMINAL",
        "lotacao_real": 6,
        "lotacao_paradigma": 5,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-153": {
        "comarca": "Curitiba",
        "unidade": "12ª VARA CÍVEL",
        "lotacao_real": 8,
        "lotacao_paradigma": 8,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-154": {
        "comarca": "Curitiba",
        "unidade": "13ª VARA CRIMINAL",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-155": {
        "comarca": "Curitiba",
        "unidade": "13º JUIZADO ESPECIAL CÍVEL E CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-156": {
        "comarca": "Curitiba",
        "unidade": "14º JUIZADO ESPECIAL CÍVEL E CRIMINAL",
        "lotacao_real": 6,
        "lotacao_paradigma": 5,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-157": {
        "comarca": "Curitiba",
        "unidade": "15ª VARA CÍVEL",
        "lotacao_real": 9,
        "lotacao_paradigma": 8,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-158": {
        "comarca": "Curitiba",
        "unidade": "18ª VARA CÍVEL",
        "lotacao_real": 7,
        "lotacao_paradigma": 8,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-159": {
        "comarca": "Curitiba",
        "unidade": "1ª VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-160": {
        "comarca": "Curitiba",
        "unidade": "1ª VARA DE FAMÍLIA",
        "lotacao_real": 9,
        "lotacao_paradigma": 5,
        "diferenca": 4,
        "status": "SUPERAVITÁRIA",
    },
    "A2-161": {
        "comarca": "Curitiba",
        "unidade": "1ª VARA DE INFRAÇÕES PENAIS CONTRA CRIANÇAS, ADOLESCENTES E IDOSOS",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-162": {
        "comarca": "Curitiba",
        "unidade": "1ª VARA DE SUCESSÕES",
        "lotacao_real": 7,
        "lotacao_paradigma": 6,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-163": {
        "comarca": "Curitiba",
        "unidade": "1ª VARA PRIVATIVA DO TRIBUNAL DO JÚRI",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-164": {
        "comarca": "Curitiba",
        "unidade": "1º JUIZADO ESPECIAL CÍVEL - MATÉRIA BANCÁRIA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-165": {
        "comarca": "Curitiba",
        "unidade": "23ª VARA CÍVEL",
        "lotacao_real": 8,
        "lotacao_paradigma": 8,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-166": {
        "comarca": "Curitiba",
        "unidade": "24ª VARA CÍVEL E EMPRESARIAL REGIONAL",
        "lotacao_real": 7,
        "lotacao_paradigma": 8,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-167": {
        "comarca": "Curitiba",
        "unidade": "25ª VARA CÍVEL E EMPRESARIAL REGIONAL",
        "lotacao_real": 7,
        "lotacao_paradigma": 8,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-168": {
        "comarca": "Curitiba",
        "unidade": "26ª VARA CÍVEL E EMPRESARIAL REGIONAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 8,
        "diferenca": -3,
        "status": "DEFICITÁRIA",
    },
    "A2-169": {
        "comarca": "Curitiba",
        "unidade": "27ª VARA CÍVEL E EMPRESARIAL REGIONAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 8,
        "diferenca": -3,
        "status": "DEFICITÁRIA",
    },
    "A2-170": {
        "comarca": "Curitiba",
        "unidade": "2ª VARA CRIMINAL",
        "lotacao_real": 6,
        "lotacao_paradigma": 5,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-171": {
        "comarca": "Curitiba",
        "unidade": "2ª VARA DE INFRAÇÕES PENAIS CONTRA CRIANÇAS, ADOLESCENTES E IDOSOS",
        "lotacao_real": 6,
        "lotacao_paradigma": 6,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-172": {
        "comarca": "Curitiba",
        "unidade": "2ª VARA DE SUCESSÕES",
        "lotacao_real": 6,
        "lotacao_paradigma": 6,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-173": {
        "comarca": "Curitiba",
        "unidade": "2ª VARA PRIVATIVA DO TRIBUNAL DO JÚRI",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-174": {
        "comarca": "Curitiba",
        "unidade": "3ª VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-175": {
        "comarca": "Curitiba",
        "unidade": "3ª VARA CÍVEL",
        "lotacao_real": 8,
        "lotacao_paradigma": 8,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-176": {
        "comarca": "Curitiba",
        "unidade": "3ª VARA DE FAMÍLIA",
        "lotacao_real": 7,
        "lotacao_paradigma": 5,
        "diferenca": 2,
        "status": "SUPERAVITÁRIA",
    },
    "A2-177": {
        "comarca": "Curitiba",
        "unidade": "3º JUIZADO DE VIOLÊNCIA DOMÉSTICA E FAMILIAR CONTRA A MULHER - CASA DA MULHER BRASILEIRA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-178": {
        "comarca": "Curitiba",
        "unidade": "3º JUIZADO ESPECIAL CÍVEL - TELECOMUNICAÇÕES",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-179": {
        "comarca": "Curitiba",
        "unidade": "4ª VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-180": {
        "comarca": "Curitiba",
        "unidade": "5ª VARA DE FAMÍLIA",
        "lotacao_real": 6,
        "lotacao_paradigma": 5,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-181": {
        "comarca": "Curitiba",
        "unidade": "5º JUIZADO ESPECIAL CÍVEL E CRIMINAL",
        "lotacao_real": 7,
        "lotacao_paradigma": 5,
        "diferenca": 2,
        "status": "SUPERAVITÁRIA",
    },
    "A2-182": {
        "comarca": "Curitiba",
        "unidade": "6ª VARA DE FAMÍLIA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-183": {
        "comarca": "Curitiba",
        "unidade": "6º JUIZADO ESPECIAL CÍVEL E CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-184": {
        "comarca": "Curitiba",
        "unidade": "7ª VARA CRIMINAL",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-185": {
        "comarca": "Curitiba",
        "unidade": "7ª VARA DE FAMÍLIA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-186": {
        "comarca": "Curitiba",
        "unidade": "8ª VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-187": {
        "comarca": "Curitiba",
        "unidade": "8º JUIZADO ESPECIAL CÍVEL E CRIMINAL",
        "lotacao_real": 6,
        "lotacao_paradigma": 5,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-188": {
        "comarca": "Curitiba",
        "unidade": "9ª VARA CRIMINAL",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-189": {
        "comarca": "Curitiba",
        "unidade": "CENTRAL DE GARANTIAS ESPECIALIZADA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-190": {
        "comarca": "Curitiba",
        "unidade": "JUIZADO ESPECIAL PUC-CAJURU",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-191": {
        "comarca": "Curitiba",
        "unidade": "SECRETARIA ESPECIALIZADA DE MOVIMENTAÇÃO PROCESSUAL DA FAZENDA PÚBLICA",
        "lotacao_real": 43,
        "lotacao_paradigma": 47,
        "diferenca": -4,
        "status": "DEFICITÁRIA",
    },
    "A2-192": {
        "comarca": "Curitiba",
        "unidade": "SECRETARIA ESPECIALIZADA EM MOVIMENTAÇÕES PROCESSUAIS DAS EXECUÇÕES PENAIS, MEDIDAS ALTERNATIVAS E CORREGEDORIA DOS PRESÍDIOS",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-193": {
        "comarca": "Curitiba",
        "unidade": "SECRETARIA ESPECIALIZADA EM MOVIMENTAÇÕES PROCESSUAIS DAS VARAS DE EXECUÇÕES FISCAIS ESTADUAIS",
        "lotacao_real": 13,
        "lotacao_paradigma": 10,
        "diferenca": 3,
        "status": "SUPERAVITÁRIA",
    },
    "A2-194": {
        "comarca": "Curitiba",
        "unidade": "SECRETARIA ESPECIALIZADA EM MOVIMENTAÇÕES PROCESSUAIS DAS VARAS DE EXECUÇÕES FISCAIS MUNICIPAIS",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-195": {
        "comarca": "Curitiba",
        "unidade": "SECRETARIA ESPECIALIZADA EM MOVIMENTAÇÕES PROCESSUAIS DOS JUIZADOS DE VIOLÊNCIA DOMÉSTICA E FAMILIAR CONTRA A MULHER",
        "lotacao_real": 31,
        "lotacao_paradigma": 29,
        "diferenca": 2,
        "status": "SUPERAVITÁRIA",
    },
    "A2-196": {
        "comarca": "Curitiba",
        "unidade": "SECRETARIA ESPECIALIZADA EM MOVIMENTAÇÕES PROCESSUAIS DOS JUIZADOS ESPECIAIS DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-197": {
        "comarca": "Curitiba",
        "unidade": "SECRETARIA UNIFICADA DA 1ª E 2ª VARAS DESCENTRALIZADAS DA CIDADE INDUSTRIAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-198": {
        "comarca": "Curitiba",
        "unidade": "SECRETARIA UNIFICADA DA 1ª E 2ª VARAS DESCENTRALIZADAS DE SANTA FELICIDADE",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-199": {
        "comarca": "Curitiba",
        "unidade": "SECRETARIA UNIFICADA DA 1ª E 2ª VARAS DESCENTRALIZADAS DO BAIRRO NOVO (SÍTIO CERCADO)",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-200": {
        "comarca": "Curitiba",
        "unidade": "SECRETARIA UNIFICADA DA 1ª E 2ª VARAS DESCENTRALIZADAS DO BOQUEIRÃO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-201": {
        "comarca": "Curitiba",
        "unidade": "SECRETARIA UNIFICADA DA 1ª E 2ª VARAS DESCENTRALIZADAS DO PINHEIRINHO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-202": {
        "comarca": "Curitiba",
        "unidade": "SECRETARIAS DAS DIREÇÕES DOS FÓRUNS",
        "lotacao_real": 39,
        "lotacao_paradigma": 39,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-203": {
        "comarca": "Curitiba",
        "unidade": "VARA DA AUDITORIA DA JUSTIÇA MILITAR",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-204": {
        "comarca": "Curitiba",
        "unidade": "VARA DA INFÂNCIA E DA JUVENTUDE E ADOÇÃO",
        "lotacao_real": 12,
        "lotacao_paradigma": 13,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-205": {
        "comarca": "Curitiba",
        "unidade": "VARA DE ADOLESCENTES EM CONFLITO COM A LEI",
        "lotacao_real": 5,
        "lotacao_paradigma": 6,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-206": {
        "comarca": "Curitiba",
        "unidade": "VARA DE DELITOS DE TRÂNSITO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-207": {
        "comarca": "Curitiba",
        "unidade": "VARA DE REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 7,
        "lotacao_paradigma": 7,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-208": {
        "comarca": "Curiúva",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-209": {
        "comarca": "Curiúva",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-210": {
        "comarca": "Cândido de Abreu",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-211": {
        "comarca": "Cândido de Abreu",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-212": {
        "comarca": "Dois Vizinhos",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-213": {
        "comarca": "Dois Vizinhos",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-214": {
        "comarca": "Engenheiro Beltrão",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-215": {
        "comarca": "Engenheiro Beltrão",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-216": {
        "comarca": "Faxinai",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-217": {
        "comarca": "Faxinai",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-218": {
        "comarca": "Faxinal",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-219": {
        "comarca": "Fazenda Rio Grande",
        "unidade": "2ª VARA CÍVEL E DA FAZENDA PÚBLICA",
        "lotacao_real": 3,
        "lotacao_paradigma": 7,
        "diferenca": -4,
        "status": "DEFICITÁRIA",
    },
    "A2-220": {
        "comarca": "Fazenda Rio Grande",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-221": {
        "comarca": "Fazenda Rio Grande",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 10,
        "lotacao_paradigma": 10,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-222": {
        "comarca": "Fazenda Rio Grande",
        "unidade": "VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 8,
        "diferenca": -3,
        "status": "DEFICITÁRIA",
    },
    "A2-223": {
        "comarca": "Fazenda Rio Grande",
        "unidade": "VARA DE FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 8,
        "diferenca": -3,
        "status": "DEFICITÁRIA",
    },
    "A2-224": {
        "comarca": "Formosa do Oeste",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-225": {
        "comarca": "Formosa do Oeste",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-226": {
        "comarca": "Foz do Iguaçu",
        "unidade": "1ª VARA CRIMINAL",
        "lotacao_real": 6,
        "lotacao_paradigma": 5,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-227": {
        "comarca": "Foz do Iguaçu",
        "unidade": "1ª VARA DA FAZENDA PÚBLICA",
        "lotacao_real": 6,
        "lotacao_paradigma": 5,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-228": {
        "comarca": "Foz do Iguaçu",
        "unidade": "1º JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 6,
        "lotacao_paradigma": 5,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-229": {
        "comarca": "Foz do Iguaçu",
        "unidade": "2ª VARA CRIMINAL",
        "lotacao_real": 6,
        "lotacao_paradigma": 5,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-230": {
        "comarca": "Foz do Iguaçu",
        "unidade": "2ª VARA DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-231": {
        "comarca": "Foz do Iguaçu",
        "unidade": "2ª VARA DE FAMÍLIA E SUCESSÕES E ACIDENTES DO TRABALHO",
        "lotacao_real": 7,
        "lotacao_paradigma": 6,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-232": {
        "comarca": "Foz do Iguaçu",
        "unidade": "2º JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 6,
        "lotacao_paradigma": 5,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-233": {
        "comarca": "Foz do Iguaçu",
        "unidade": "3ª VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-234": {
        "comarca": "Foz do Iguaçu",
        "unidade": "3º JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-235": {
        "comarca": "Foz do Iguaçu",
        "unidade": "JUIZADO DE VIOLÊNCIA DOMÉSTICA E FAMILIAR CONTRA A MULHER, VARA DE CRIMES CONTRA CRIANÇAS, ADOLESCENTES E IDOSOS",
        "lotacao_real": 8,
        "lotacao_paradigma": 9,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-236": {
        "comarca": "Foz do Iguaçu",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 23,
        "lotacao_paradigma": 23,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-237": {
        "comarca": "Foz do Iguaçu",
        "unidade": "VARA DA INFÂNCIA E JUVENTUDE",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-238": {
        "comarca": "Foz do Iguaçu",
        "unidade": "VARA DE EXECUÇÕES PENAIS E CORREGEDORIA DOS PRESÍDIOS",
        "lotacao_real": 7,
        "lotacao_paradigma": 8,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-239": {
        "comarca": "Francisco Beltrão",
        "unidade": "1ª VARA CÍVEL E DA FAZENDA PÚBLICA",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-240": {
        "comarca": "Francisco Beltrão",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 8,
        "lotacao_paradigma": 7,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-241": {
        "comarca": "Francisco Beltrão",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 13,
        "lotacao_paradigma": 13,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-242": {
        "comarca": "Francisco Beltrão",
        "unidade": "VARA CRIMINAL",
        "lotacao_real": 7,
        "lotacao_paradigma": 9,
        "diferenca": -2,
        "status": "DEFICITÁRIA",
    },
    "A2-243": {
        "comarca": "Francisco Beltrão",
        "unidade": "VARA DE EXECUÇÕES PENAIS E CORREGEDORIA DOS PRESÍDIOS",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-244": {
        "comarca": "Francisco Beltrão",
        "unidade": "VARA DE FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 4,
        "lotacao_paradigma": 6,
        "diferenca": -2,
        "status": "DEFICITÁRIA",
    },
    "A2-245": {
        "comarca": "Goioerê",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 4,
        "lotacao_paradigma": 4,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-246": {
        "comarca": "Goioerê",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-247": {
        "comarca": "Goioerê",
        "unidade": "VARA CÍVEL, DA FAZENDA PÚBLICA, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL, JUIZADO ESPECIAL CÍVEL E JUIZADO ESPECIAL DA FAZENDA PÚBLICA",
        "lotacao_real": 6,
        "lotacao_paradigma": 6,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-248": {
        "comarca": "Grandes Rios",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-249": {
        "comarca": "Grandes Rios",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-250": {
        "comarca": "Guaraniaçu",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 6,
        "lotacao_paradigma": 8,
        "diferenca": -2,
        "status": "DEFICITÁRIA",
    },
    "A2-251": {
        "comarca": "Guaraniaçu",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 4,
        "lotacao_paradigma": 4,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-252": {
        "comarca": "Guarapuava",
        "unidade": "1ª VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 6,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-253": {
        "comarca": "Guarapuava",
        "unidade": "1ª VARA CÍVEL E DA FAZENDA PÚBLICA",
        "lotacao_real": 9,
        "lotacao_paradigma": 6,
        "diferenca": 3,
        "status": "SUPERAVITÁRIA",
    },
    "A2-254": {
        "comarca": "Guarapuava",
        "unidade": "1º JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 6,
        "lotacao_paradigma": 5,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-255": {
        "comarca": "Guarapuava",
        "unidade": "2ª VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-256": {
        "comarca": "Guarapuava",
        "unidade": "2º JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 6,
        "lotacao_paradigma": 5,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-257": {
        "comarca": "Guarapuava",
        "unidade": "3ª VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-258": {
        "comarca": "Guarapuava",
        "unidade": "3ª VARA CÍVEL E DA FAZENDA PÚBLICA",
        "lotacao_real": 8,
        "lotacao_paradigma": 6,
        "diferenca": 2,
        "status": "SUPERAVITÁRIA",
    },
    "A2-259": {
        "comarca": "Guarapuava",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 23,
        "lotacao_paradigma": 23,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-260": {
        "comarca": "Guarapuava",
        "unidade": "VARA DA INFÂNCIA E JUVENTUDE",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-261": {
        "comarca": "Guarapuava",
        "unidade": "VARA DE EXECUÇÕES PENAIS E CORREGEDORIA DOS PRESÍDIOS",
        "lotacao_real": 6,
        "lotacao_paradigma": 5,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-262": {
        "comarca": "Guarapuava",
        "unidade": "VARA DE FAMÍLIA E SUCESSÕES, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 8,
        "lotacao_paradigma": 8,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-263": {
        "comarca": "Guaratuba",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 6,
        "lotacao_paradigma": 6,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-264": {
        "comarca": "Guaratuba",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-265": {
        "comarca": "Guaíra",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 4,
        "lotacao_paradigma": 4,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-266": {
        "comarca": "Guaíra",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 7,
        "lotacao_paradigma": 8,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-267": {
        "comarca": "Guaíra",
        "unidade": "VARA CÍVEL, DA FAZENDA PÚBLICA, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL, JUIZADO ESPECIAL CÍVEL E JUIZADO ESPECIAL DA FAZENDA PÚBLICA",
        "lotacao_real": 7,
        "lotacao_paradigma": 5,
        "diferenca": 2,
        "status": "SUPERAVITÁRIA",
    },
    "A2-268": {
        "comarca": "Ibaiti",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-269": {
        "comarca": "Ibiporã",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-270": {
        "comarca": "Ibiporã",
        "unidade": "VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-271": {
        "comarca": "Ibiporã",
        "unidade": "VARA DE FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 3,
        "lotacao_paradigma": 5,
        "diferenca": -2,
        "status": "DEFICITÁRIA",
    },
    "A2-272": {
        "comarca": "Icaraíma",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-273": {
        "comarca": "Imbituva",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 7,
        "lotacao_paradigma": 9,
        "diferenca": -2,
        "status": "DEFICITÁRIA",
    },
    "A2-274": {
        "comarca": "Ipiranga",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-275": {
        "comarca": "Iporã",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-276": {
        "comarca": "Irati",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL, DA FAZENDA PÚBLICA E VARA DE FAMÍLIA E SUCESSÕES",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-277": {
        "comarca": "Irati",
        "unidade": "SECRETARIA ESPECIALIZADA DE MOVIMENTAÇÃO PROCESSUAL DAS VARAS CÍVEIS",
        "lotacao_real": 7,
        "lotacao_paradigma": 10,
        "diferenca": -3,
        "status": "DEFICITÁRIA",
    },
    "A2-278": {
        "comarca": "Irati",
        "unidade": "VARA CRIMINAL E INFÂNCIA E JUVENTUDE",
        "lotacao_real": 6,
        "lotacao_paradigma": 6,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-279": {
        "comarca": "Iretama",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-280": {
        "comarca": "Ivaiporã",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-281": {
        "comarca": "Ivaiporã",
        "unidade": "VARA CRIMINAL, INFÂNCIA E JUVENTUDE E FAMÍLIA E SUCESSÕES",
        "lotacao_real": 6,
        "lotacao_paradigma": 7,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-282": {
        "comarca": "Jaca Rezinho",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-283": {
        "comarca": "Jacarezinho",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-284": {
        "comarca": "Jacarezinho",
        "unidade": "VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 6,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-285": {
        "comarca": "Jacarezinho",
        "unidade": "VARA CÍVEL E DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-286": {
        "comarca": "Jacarezinho",
        "unidade": "VARA DE FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-287": {
        "comarca": "Jagua Riaíva",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-288": {
        "comarca": "Jaguapitã",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-289": {
        "comarca": "Jaguapitã",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 2,
        "lotacao_paradigma": 2,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-290": {
        "comarca": "Jaguariaíva",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-291": {
        "comarca": "Jandaia do Sul",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 4,
        "lotacao_paradigma": 4,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-292": {
        "comarca": "Jandaia do Sul",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 7,
        "lotacao_paradigma": 5,
        "diferenca": 2,
        "status": "SUPERAVITÁRIA",
    },
    "A2-293": {
        "comarca": "Jandaia do Sul",
        "unidade": "VARA CÍVEL, DA FAZENDA PÚBLICA, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL, JUIZADO ESPECIAL CÍVEL E JUIZADO ESPECIAL DA FAZENDA PÚBLICA",
        "lotacao_real": 8,
        "lotacao_paradigma": 8,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-294": {
        "comarca": "Joaquim Távora",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-295": {
        "comarca": "Joaquim Távora",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-296": {
        "comarca": "Lapa",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-297": {
        "comarca": "Lapa",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-298": {
        "comarca": "Lapa",
        "unidade": "VARA CRIMINAL, INFÂNCIA E JUVENTUDE E FAMÍLIA E SUCESSÕES",
        "lotacao_real": 8,
        "lotacao_paradigma": 8,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-299": {
        "comarca": "Laranjeiras do Sul",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 7,
        "lotacao_paradigma": 7,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-300": {
        "comarca": "Laranjeiras do Sul",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-301": {
        "comarca": "Lbaiti",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-302": {
        "comarca": "Lbiporã",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-303": {
        "comarca": "Lbiporã",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-304": {
        "comarca": "Lbiporã",
        "unidade": "VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-305": {
        "comarca": "Lbiporã",
        "unidade": "VARA DE FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-306": {
        "comarca": "Lcaraíma",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-307": {
        "comarca": "Lcaraíma",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-308": {
        "comarca": "Lmbituva",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-309": {
        "comarca": "Lmbituva",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-310": {
        "comarca": "Loanda",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 4,
        "lotacao_paradigma": 4,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-311": {
        "comarca": "Loanda",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-312": {
        "comarca": "Londrina",
        "unidade": "11ª VARA CÍVEL E EMPRESARIAL REGIONAL",
        "lotacao_real": 6,
        "lotacao_paradigma": 0,
        "diferenca": 6,
        "status": "SUPERAVITÁRIA",
    },
    "A2-313": {
        "comarca": "Londrina",
        "unidade": "1ª VARA DE EXECUÇÕES FISCAIS",
        "lotacao_real": 6,
        "lotacao_paradigma": 5,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-314": {
        "comarca": "Londrina",
        "unidade": "1º JUIZADO DE VIOLÊNCIA DOMÉSTICA E FAMILIAR CONTRA A MULHER E VARA DE CRIMES CONTRA CRIANÇAS, ADOLESCENTES E IDOSOS",
        "lotacao_real": 9,
        "lotacao_paradigma": 8,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-315": {
        "comarca": "Londrina",
        "unidade": "1º JUIZADO ESPECIAL DA FAZENDA PÚBLICA",
        "lotacao_real": 7,
        "lotacao_paradigma": 5,
        "diferenca": 2,
        "status": "SUPERAVITÁRIA",
    },
    "A2-316": {
        "comarca": "Londrina",
        "unidade": "2ª VARA CRIMINAL",
        "lotacao_real": 6,
        "lotacao_paradigma": 5,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-317": {
        "comarca": "Londrina",
        "unidade": "2ª VARA DE EXECUÇÕES FISCAIS",
        "lotacao_real": 6,
        "lotacao_paradigma": 5,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-318": {
        "comarca": "Londrina",
        "unidade": "2º JUIZADO DE VIOLÊNCIA DOMÉSTICA E FAMILIAR CONTRA A MULHER E VARA DE CRIMES CONTRA CRIANÇAS, ADOLESCENTES E IDOSOS",
        "lotacao_real": 8,
        "lotacao_paradigma": 8,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-319": {
        "comarca": "Londrina",
        "unidade": "2º JUIZADO ESPECIAL CÍVEL E CRIMINAL",
        "lotacao_real": 8,
        "lotacao_paradigma": 7,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-320": {
        "comarca": "Londrina",
        "unidade": "3ª VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-321": {
        "comarca": "Londrina",
        "unidade": "3ª VARA DE FAMÍLIA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-322": {
        "comarca": "Londrina",
        "unidade": "3º JUIZADO ESPECIAL CÍVEL E CRIMINAL",
        "lotacao_real": 7,
        "lotacao_paradigma": 7,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-323": {
        "comarca": "Londrina",
        "unidade": "4ª VARA CRIMINAL",
        "lotacao_real": 6,
        "lotacao_paradigma": 5,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-324": {
        "comarca": "Londrina",
        "unidade": "4º JUIZADO ESPECIAL CÍVEL E CRIMINAL",
        "lotacao_real": 7,
        "lotacao_paradigma": 7,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-325": {
        "comarca": "Londrina",
        "unidade": "5ª VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-326": {
        "comarca": "Londrina",
        "unidade": "5º JUIZADO ESPECIAL CÍVEL E CRIMINAL",
        "lotacao_real": 8,
        "lotacao_paradigma": 7,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-327": {
        "comarca": "Londrina",
        "unidade": "6º JUIZADO ESPECIAL CÍVEL E CRIMINAL",
        "lotacao_real": 7,
        "lotacao_paradigma": 7,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-328": {
        "comarca": "Londrina",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 51,
        "lotacao_paradigma": 51,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-329": {
        "comarca": "Londrina",
        "unidade": "5ª VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-330": {
        "comarca": "Londrina",
        "unidade": "5º JUIZADO ESPECIAL CÍVEL E CRIMINAL",
        "lotacao_real": 8,
        "lotacao_paradigma": 7,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-331": {
        "comarca": "Londrina",
        "unidade": "VARA DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-332": {
        "comarca": "Londrina",
        "unidade": "VARA DA INFÂNCIA E JUVENTUDE",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-333": {
        "comarca": "Londrina",
        "unidade": "VARA DE EXECUÇÕES PENAIS, MEDIDAS ALTERNATIVAS E CORREGEDORIA DOS PRESÍDIOS",
        "lotacao_real": 8,
        "lotacao_paradigma": 11,
        "diferenca": -3,
        "status": "DEFICITÁRIA",
    },
    "A2-334": {
        "comarca": "Londrina",
        "unidade": "VARA DE SUCESSÕES, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 7,
        "diferenca": -2,
        "status": "DEFICITÁRIA",
    },
    "A2-335": {
        "comarca": "Londrina",
        "unidade": "VARA PRIVATIVA DO TRIBUNAL DO JÚRI",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-336": {
        "comarca": "Lpiranga",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-337": {
        "comarca": "Lpiranga",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-338": {
        "comarca": "Lporã",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-339": {
        "comarca": "Lporã",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-340": {
        "comarca": "Lrati",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-341": {
        "comarca": "Lretama",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-342": {
        "comarca": "Lretama",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-343": {
        "comarca": "Lvaiporã",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-344": {
        "comarca": "Lvaiporã",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-345": {
        "comarca": "Lvaiporã",
        "unidade": "VARA CRIMINAL, INFÂNCIA E JUVENTUDE E FAMÍLIA E SUCESSÕES",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-346": {
        "comarca": "Mallet",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-347": {
        "comarca": "Mallet",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-348": {
        "comarca": "Mambo Rê",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-349": {
        "comarca": "Mamborê",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 3,
        "lotacao_paradigma": 5,
        "diferenca": -2,
        "status": "DEFICITÁRIA",
    },
    "A2-350": {
        "comarca": "Mandaguari",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-351": {
        "comarca": "Mandaguari",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-352": {
        "comarca": "Mandaguaçu",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 9,
        "lotacao_paradigma": 10,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-353": {
        "comarca": "Mandaguaçu",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 2,
        "lotacao_paradigma": 2,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-354": {
        "comarca": "Mangueirinha",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 6,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-355": {
        "comarca": "Mangueirinha",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 4,
        "lotacao_paradigma": 4,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-356": {
        "comarca": "Manoel Ribas",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-357": {
        "comarca": "Manoel Ribas",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 2,
        "lotacao_paradigma": 2,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-358": {
        "comarca": "Marechal Cândido Rondon",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-359": {
        "comarca": "Marechal Cândido Rondon",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 6,
        "lotacao_paradigma": 6,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-360": {
        "comarca": "Marechal Cândido Rondon",
        "unidade": "VARA CRIMINAL",
        "lotacao_real": 6,
        "lotacao_paradigma": 8,
        "diferenca": -2,
        "status": "DEFICITÁRIA",
    },
    "A2-361": {
        "comarca": "Marechal Cândido Rondon",
        "unidade": "VARA DE FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 6,
        "lotacao_paradigma": 5,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-362": {
        "comarca": "Marialva",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 4,
        "lotacao_paradigma": 4,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-363": {
        "comarca": "Marialva",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-364": {
        "comarca": "Marilândia do Sul",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 4,
        "lotacao_paradigma": 7,
        "diferenca": -3,
        "status": "DEFICITÁRIA",
    },
    "A2-365": {
        "comarca": "Marilândia do Sul",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-366": {
        "comarca": "Maringá",
        "unidade": "1ª VARA CRIMINAL",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-367": {
        "comarca": "Maringá",
        "unidade": "1ª VARA DA FAZENDA PÚBLICA",
        "lotacao_real": 9,
        "lotacao_paradigma": 8,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-368": {
        "comarca": "Maringá",
        "unidade": "1ª VARA DE FAMÍLIA E SUCESSÕES, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 7,
        "lotacao_paradigma": 7,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-369": {
        "comarca": "Maringá",
        "unidade": "1º JUIZADO ESPECIAL CÍVEL E CRIMINAL",
        "lotacao_real": 9,
        "lotacao_paradigma": 8,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-370": {
        "comarca": "Maringá",
        "unidade": "2ª VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-371": {
        "comarca": "Maringá",
        "unidade": "2ª VARA DA FAZENDA PÚBLICA",
        "lotacao_real": 10,
        "lotacao_paradigma": 8,
        "diferenca": 2,
        "status": "SUPERAVITÁRIA",
    },
    "A2-372": {
        "comarca": "Maringá",
        "unidade": "2ª VARA DE FAMÍLIA E SUCESSÕES E ACIDENTES DO TRABALHO",
        "lotacao_real": 7,
        "lotacao_paradigma": 8,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-373": {
        "comarca": "Maringá",
        "unidade": "2º JUIZADO ESPECIAL CÍVEL E CRIMINAL",
        "lotacao_real": 10,
        "lotacao_paradigma": 8,
        "diferenca": 2,
        "status": "SUPERAVITÁRIA",
    },
    "A2-374": {
        "comarca": "Maringá",
        "unidade": "3ª VARA CRIMINAL",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-375": {
        "comarca": "Maringá",
        "unidade": "3ª VARA CÍVEL E EMPRESARIAL REGIONAL",
        "lotacao_real": 8,
        "lotacao_paradigma": 11,
        "diferenca": -3,
        "status": "DEFICITÁRIA",
    },
    "A2-376": {
        "comarca": "Maringá",
        "unidade": "3º JUIZADO ESPECIAL CÍVEL E CRIMINAL",
        "lotacao_real": 10,
        "lotacao_paradigma": 8,
        "diferenca": 2,
        "status": "SUPERAVITÁRIA",
    },
    "A2-377": {
        "comarca": "Maringá",
        "unidade": "4ª VARA CRIMINAL",
        "lotacao_real": 6,
        "lotacao_paradigma": 5,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-378": {
        "comarca": "Maringá",
        "unidade": "4ª VARA CÍVEL",
        "lotacao_real": 9,
        "lotacao_paradigma": 9,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-379": {
        "comarca": "Maringá",
        "unidade": "4º JUIZADO ESPECIAL CÍVEL E CRIMINAL",
        "lotacao_real": 10,
        "lotacao_paradigma": 8,
        "diferenca": 2,
        "status": "SUPERAVITÁRIA",
    },
    "A2-380": {
        "comarca": "Maringá",
        "unidade": "5º JUIZADO ESPECIAL DA FAZENDA PÚBLICA",
        "lotacao_real": 6,
        "lotacao_paradigma": 6,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-381": {
        "comarca": "Maringá",
        "unidade": "7ª VARA CÍVEL",
        "lotacao_real": 8,
        "lotacao_paradigma": 9,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-382": {
        "comarca": "Maringá",
        "unidade": "JUIZADO DE VIOLÊNCIA DOMÉSTICA E FAMILIAR CONTRA A MULHER E VARA DE CRIMES CONTRA CRIANÇAS, ADOLESCENTES E IDOSOS",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-383": {
        "comarca": "Maringá",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 28,
        "lotacao_paradigma": 28,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-384": {
        "comarca": "Maringá",
        "unidade": "5º JUIZADO ESPECIAL DA FAZENDA PÚBLICA",
        "lotacao_real": 6,
        "lotacao_paradigma": 6,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-385": {
        "comarca": "Maringá",
        "unidade": "VARA DA INFÂNCIA E JUVENTUDE",
        "lotacao_real": 7,
        "lotacao_paradigma": 7,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-386": {
        "comarca": "Maringá",
        "unidade": "VARA DE EXECUÇÕES PENAIS, MEDIDAS ALTERNATIVAS E CORREGEDORIA DOS PRESÍDIOS",
        "lotacao_real": 8,
        "lotacao_paradigma": 6,
        "diferenca": 2,
        "status": "SUPERAVITÁRIA",
    },
    "A2-387": {
        "comarca": "Marmeleiro",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 8,
        "lotacao_paradigma": 9,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-388": {
        "comarca": "Marmeleiro",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 4,
        "lotacao_paradigma": 4,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-389": {
        "comarca": "Marretes",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-390": {
        "comarca": "Marretes",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-391": {
        "comarca": "Matelândia",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-392": {
        "comarca": "Matelândia",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-393": {
        "comarca": "Matinhos",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 6,
        "lotacao_paradigma": 6,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-394": {
        "comarca": "Matinhos",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-395": {
        "comarca": "Medianeira",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 4,
        "lotacao_paradigma": 4,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-396": {
        "comarca": "Medianeira",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-397": {
        "comarca": "Morretes",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-398": {
        "comarca": "Nova Aurora",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 6,
        "lotacao_paradigma": 8,
        "diferenca": -2,
        "status": "DEFICITÁRIA",
    },
    "A2-399": {
        "comarca": "Nova Aurora",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 4,
        "lotacao_paradigma": 4,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-400": {
        "comarca": "Nova Esperança",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-401": {
        "comarca": "Nova Esperança",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 4,
        "lotacao_paradigma": 4,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-402": {
        "comarca": "Nova Esperança",
        "unidade": "VARA CRIMINAL, INFÂNCIA E JUVENTUDE E FAMÍLIA E SUCESSÕES",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-403": {
        "comarca": "Nova Esperança",
        "unidade": "VARA CÍVEL, DA FAZENDA PÚBLICA, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-404": {
        "comarca": "Nova Fátima",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-405": {
        "comarca": "Nova Fátima",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-406": {
        "comarca": "Nova Londrina",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-407": {
        "comarca": "Nova Londrina",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 2,
        "lotacao_paradigma": 2,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-408": {
        "comarca": "Ortigueira",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-409": {
        "comarca": "Ortigueira",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-410": {
        "comarca": "Paiçandu",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-411": {
        "comarca": "Paiçandu",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-412": {
        "comarca": "Paiçandu",
        "unidade": "VARA CÍVEL, DA FAZENDA PÚBLICA, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL, JUIZADO ESPECIAL CÍVEL E JUIZADO ESPECIAL DA FAZENDA PÚBLICA",
        "lotacao_real": 6,
        "lotacao_paradigma": 7,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-413": {
        "comarca": "Palatina",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-414": {
        "comarca": "Palatina",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-415": {
        "comarca": "Palmas",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 7,
        "lotacao_paradigma": 7,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-416": {
        "comarca": "Palmas",
        "unidade": "VARA CRIMINAL E DA INFÂNCIA E JUVENTUDE",
        "lotacao_real": 5,
        "lotacao_paradigma": 8,
        "diferenca": -3,
        "status": "DEFICITÁRIA",
    },
    "A2-417": {
        "comarca": "Palmas",
        "unidade": "VARA DE FAMÍLIA E SUCESSÕES, JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-418": {
        "comarca": "Palmeira",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-419": {
        "comarca": "Palmeira",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-420": {
        "comarca": "Palmital",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-421": {
        "comarca": "Palmital",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 4,
        "lotacao_paradigma": 4,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-422": {
        "comarca": "Palotina",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-423": {
        "comarca": "Paranacity",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-424": {
        "comarca": "Paranacity",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-425": {
        "comarca": "Paranaguá",
        "unidade": "1ª VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 6,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-426": {
        "comarca": "Paranaguá",
        "unidade": "1ª VARA CÍVEL",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-427": {
        "comarca": "Paranaguá",
        "unidade": "2ª VARA CRIMINAL",
        "lotacao_real": 4,
        "lotacao_paradigma": 6,
        "diferenca": -2,
        "status": "DEFICITÁRIA",
    },
    "A2-428": {
        "comarca": "Paranaguá",
        "unidade": "2ª VARA CÍVEL",
        "lotacao_real": 2,
        "lotacao_paradigma": 5,
        "diferenca": -3,
        "status": "DEFICITÁRIA",
    },
    "A2-429": {
        "comarca": "Paranaguá",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-430": {
        "comarca": "Paranaguá",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 12,
        "lotacao_paradigma": 12,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-431": {
        "comarca": "Paranaguá",
        "unidade": "VARA DA FAZENDA PÚBLICA",
        "lotacao_real": 11,
        "lotacao_paradigma": 11,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-432": {
        "comarca": "Paranaguá",
        "unidade": "VARA DA INFÂNCIA E JUVENTUDE, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-433": {
        "comarca": "Paranaguá",
        "unidade": "VARA DE FAMÍLIA E SUCESSÕES",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-434": {
        "comarca": "Paranavaí",
        "unidade": "1ª VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-435": {
        "comarca": "Paranavaí",
        "unidade": "2ª VARA CRIMINAL",
        "lotacao_real": 6,
        "lotacao_paradigma": 6,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-436": {
        "comarca": "Paranavaí",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 9,
        "lotacao_paradigma": 7,
        "diferenca": 2,
        "status": "SUPERAVITÁRIA",
    },
    "A2-437": {
        "comarca": "Paranavaí",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 10,
        "lotacao_paradigma": 10,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-438": {
        "comarca": "Paranavaí",
        "unidade": "VARA DE FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 6,
        "lotacao_paradigma": 6,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-439": {
        "comarca": "Paraíso do Norte",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-440": {
        "comarca": "Paraíso do Norte",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 4,
        "lotacao_paradigma": 4,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-441": {
        "comarca": "Pato Branco",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 6,
        "lotacao_paradigma": 5,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-442": {
        "comarca": "Pato Branco",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 9,
        "lotacao_paradigma": 9,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-443": {
        "comarca": "Pato Branco",
        "unidade": "VARA CRIMINAL",
        "lotacao_real": 9,
        "lotacao_paradigma": 9,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-444": {
        "comarca": "Pato Branco",
        "unidade": "VARA DE FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 6,
        "lotacao_paradigma": 6,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-445": {
        "comarca": "Peabiru",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 6,
        "lotacao_paradigma": 7,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-446": {
        "comarca": "Peabiru",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 4,
        "lotacao_paradigma": 4,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-447": {
        "comarca": "Pinhais",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-448": {
        "comarca": "Pinhais",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 8,
        "lotacao_paradigma": 8,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-449": {
        "comarca": "Pinhais",
        "unidade": "VARA CRIMINAL",
        "lotacao_real": 6,
        "lotacao_paradigma": 6,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-450": {
        "comarca": "Pinhais",
        "unidade": "VARA DE FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 6,
        "lotacao_paradigma": 6,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-451": {
        "comarca": "Pinhão",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 4,
        "lotacao_paradigma": 4,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-452": {
        "comarca": "Pinhão",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-453": {
        "comarca": "Pinhão",
        "unidade": "VARA CÍVEL, DA FAZENDA PÚBLICA, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL, JUIZADO ESPECIAL CÍVEL E JUIZADO ESPECIAL DA FAZENDA PÚBLICA",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-454": {
        "comarca": "Piraquara",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-455": {
        "comarca": "Piraquara",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 8,
        "lotacao_paradigma": 8,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-456": {
        "comarca": "Piraquara",
        "unidade": "VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 8,
        "diferenca": -3,
        "status": "DEFICITÁRIA",
    },
    "A2-457": {
        "comarca": "Piraquara",
        "unidade": "VARA CÍVEL E DA FAZENDA PÚBLICA",
        "lotacao_real": 3,
        "lotacao_paradigma": 5,
        "diferenca": -2,
        "status": "DEFICITÁRIA",
    },
    "A2-458": {
        "comarca": "Piraquara",
        "unidade": "VARA DE FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 4,
        "lotacao_paradigma": 6,
        "diferenca": -2,
        "status": "DEFICITÁRIA",
    },
    "A2-459": {
        "comarca": "Piraí do Sul",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 4,
        "lotacao_paradigma": 6,
        "diferenca": -2,
        "status": "DEFICITÁRIA",
    },
    "A2-460": {
        "comarca": "Piraí do Sul",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-461": {
        "comarca": "Pitanga",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-462": {
        "comarca": "Pitanga",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-463": {
        "comarca": "Ponta Grossa",
        "unidade": "1ª VARA CRIMINAL",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-464": {
        "comarca": "Ponta Grossa",
        "unidade": "1ª VARA CÍVEL E EMPRESARIAL REGIONAL",
        "lotacao_real": 6,
        "lotacao_paradigma": 7,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-465": {
        "comarca": "Ponta Grossa",
        "unidade": "1ª VARA DA FAZENDA PÚBLICA",
        "lotacao_real": 7,
        "lotacao_paradigma": 7,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-466": {
        "comarca": "Ponta Grossa",
        "unidade": "1º JUIZADO DE VIOLÊNCIA DOMÉSTICA E FAMILIAR CONTRA A MULHER, VARA DE CRIMES CONTRA CRIANÇAS, ADOLESCENTES E IDOSOS",
        "lotacao_real": 9,
        "lotacao_paradigma": 13,
        "diferenca": -4,
        "status": "DEFICITÁRIA",
    },
    "A2-467": {
        "comarca": "Ponta Grossa",
        "unidade": "1º JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 6,
        "lotacao_paradigma": 6,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-468": {
        "comarca": "Ponta Grossa",
        "unidade": "2ª VARA DA FAZENDA PÚBLICA",
        "lotacao_real": 8,
        "lotacao_paradigma": 7,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-469": {
        "comarca": "Ponta Grossa",
        "unidade": "2º JUIZADO DE VIOLÊNCIA DOMÉSTICA E FAMILIAR CONTRA A MULHER, VARA DE CRIMES CONTRA CRIANÇAS, ADOLESCENTES E IDOSOS",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-470": {
        "comarca": "Ponta Grossa",
        "unidade": "2º JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 6,
        "lotacao_paradigma": 6,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-471": {
        "comarca": "Ponta Grossa",
        "unidade": "3ª VARA CRIMINAL",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-472": {
        "comarca": "Ponta Grossa",
        "unidade": "3º JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 8,
        "lotacao_paradigma": 6,
        "diferenca": 2,
        "status": "SUPERAVITÁRIA",
    },
    "A2-473": {
        "comarca": "Ponta Grossa",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 28,
        "lotacao_paradigma": 28,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-474": {
        "comarca": "Ponta Grossa",
        "unidade": "VARA DA INFÂNCIA E JUVENTUDE",
        "lotacao_real": 7,
        "lotacao_paradigma": 9,
        "diferenca": -2,
        "status": "DEFICITÁRIA",
    },
    "A2-475": {
        "comarca": "Ponta Grossa",
        "unidade": "VARA DE EXECUÇÕES PENAIS E CORREGEDORIA DOS PRESÍDIOS",
        "lotacao_real": 5,
        "lotacao_paradigma": 6,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-476": {
        "comarca": "Pontal do Paraná",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-477": {
        "comarca": "Pontal do Paraná",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CÍVEL E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-478": {
        "comarca": "Pontal do Paraná",
        "unidade": "VARA CÍVEL, DA FAZENDA PÚBLICA, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL E JUIZADO ESPECIAL DA FAZENDA PÚBLICA",
        "lotacao_real": 6,
        "lotacao_paradigma": 6,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-479": {
        "comarca": "Porecatu",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-480": {
        "comarca": "Porecatu",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-481": {
        "comarca": "Porecatu",
        "unidade": "VARA CÍVEL, DA FAZENDA PÚBLICA, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL, JUIZADO ESPECIAL CÍVEL E JUIZADO ESPECIAL DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-482": {
        "comarca": "Primeiro de Maio",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-483": {
        "comarca": "Primeiro de Maio",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-484": {
        "comarca": "Prudentópolis",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-485": {
        "comarca": "Prudentópolis",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 6,
        "lotacao_paradigma": 5,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-486": {
        "comarca": "Prudentópolis",
        "unidade": "VARA CÍVEL, DA FAZENDA PÚBLICA, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL, JUIZADO ESPECIAL CÍVEL E JUIZADO ESPECIAL DA FAZENDA PÚBLICA",
        "lotacao_real": 6,
        "lotacao_paradigma": 5,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-487": {
        "comarca": "Pérola",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-488": {
        "comarca": "Pérola",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-489": {
        "comarca": "Quatro Barras",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-490": {
        "comarca": "Quatro Barras",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 4,
        "lotacao_paradigma": 4,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-491": {
        "comarca": "Quedas do Iguaçu",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-492": {
        "comarca": "Quedas do Iguaçu",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-493": {
        "comarca": "Quedas do Iguaçu",
        "unidade": "VARA CÍVEL, DA FAZENDA PÚBLICA, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL, JUIZADO ESPECIAL CÍVEL E JUIZADO ESPECIAL DA FAZENDA PÚBLICA",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-494": {
        "comarca": "Realeza",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-495": {
        "comarca": "Realeza",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-496": {
        "comarca": "Rebouças",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-497": {
        "comarca": "Rebouças",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-498": {
        "comarca": "Reserva",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-499": {
        "comarca": "Reserva",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-500": {
        "comarca": "Ribeirão Claro",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-501": {
        "comarca": "Ribeirão Claro",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-502": {
        "comarca": "Ribeirão do Pinhal",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-503": {
        "comarca": "Ribeirão do Pinhal",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-504": {
        "comarca": "Rio Branco do Sul",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL, DA FAZENDA PÚBLICA E FAMÍLIA E SUCESSÕES",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-505": {
        "comarca": "Rio Branco do Sul",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-506": {
        "comarca": "Rio Branco do Sul",
        "unidade": "VARA CRIMINAL E INFÂNCIA E JUVENTUDE",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-507": {
        "comarca": "Rio Negro",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 4,
        "lotacao_paradigma": 4,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-508": {
        "comarca": "Rio Negro",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-509": {
        "comarca": "Rolândia",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 7,
        "lotacao_paradigma": 5,
        "diferenca": 2,
        "status": "SUPERAVITÁRIA",
    },
    "A2-510": {
        "comarca": "Rolândia",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-511": {
        "comarca": "Rolândia",
        "unidade": "VARA CRIMINAL E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-512": {
        "comarca": "Rolândia",
        "unidade": "VARA DA INFÂNCIA E JUVENTUDE, FAMÍLIA E SUCESSÕES, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-513": {
        "comarca": "Salto do Lontra",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-514": {
        "comarca": "Salto do Lontra",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 4,
        "lotacao_paradigma": 4,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-515": {
        "comarca": "Santa Fé",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 7,
        "lotacao_paradigma": 8,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-516": {
        "comarca": "Santa Fé",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 4,
        "lotacao_paradigma": 4,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-517": {
        "comarca": "Santa Helena",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-518": {
        "comarca": "Santa Helena",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 4,
        "lotacao_paradigma": 4,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-519": {
        "comarca": "Santa Isabel do Ivaí",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-520": {
        "comarca": "Santa Isabel do Lvaí",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-521": {
        "comarca": "Santa Mariana",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 6,
        "lotacao_paradigma": 5,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-522": {
        "comarca": "Santa Mariana",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-523": {
        "comarca": "Santo Antônio da Platina",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-524": {
        "comarca": "Santo Antônio da Platina",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-525": {
        "comarca": "Santo Antônio da Platina",
        "unidade": "VARA CRIMINAL, INFÂNCIA E JUVENTUDE E FAMÍLIA E SUCESSÕES",
        "lotacao_real": 6,
        "lotacao_paradigma": 7,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-526": {
        "comarca": "Santo Antônio do Sudoeste",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-527": {
        "comarca": "Santo Antônio do Sudoeste",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 4,
        "lotacao_paradigma": 4,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-528": {
        "comarca": "Sarandi",
        "unidade": "1ª VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-529": {
        "comarca": "Sarandi",
        "unidade": "2ª VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-530": {
        "comarca": "Sarandi",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-531": {
        "comarca": "Sarandi",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 8,
        "lotacao_paradigma": 8,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-532": {
        "comarca": "Sarandi",
        "unidade": "VARA DE FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-533": {
        "comarca": "Sengés",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-534": {
        "comarca": "Sengés",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-535": {
        "comarca": "Sertanópolis",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-536": {
        "comarca": "Sertanópolis",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-537": {
        "comarca": "Siqueira Campos",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-538": {
        "comarca": "Siqueira Campos",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-539": {
        "comarca": "São Jerônimo da Serra",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 3,
        "lotacao_paradigma": 5,
        "diferenca": -2,
        "status": "DEFICITÁRIA",
    },
    "A2-540": {
        "comarca": "São Jerônimo da Serra",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-541": {
        "comarca": "São José dos Pinhais",
        "unidade": "1ª VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-542": {
        "comarca": "São José dos Pinhais",
        "unidade": "1º JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-543": {
        "comarca": "São José dos Pinhais",
        "unidade": "2ª VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 6,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-544": {
        "comarca": "São José dos Pinhais",
        "unidade": "2º JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-545": {
        "comarca": "São José dos Pinhais",
        "unidade": "3ª VARA CÍVEL",
        "lotacao_real": 7,
        "lotacao_paradigma": 9,
        "diferenca": -2,
        "status": "DEFICITÁRIA",
    },
    "A2-546": {
        "comarca": "São José dos Pinhais",
        "unidade": "JUIZADO DE VIOLÊNCIA DOMÉSTICA E FAMILIAR CONTRA A MULHER, VARA DE CRIMES CONTRA CRIANÇAS, ADOLESCENTES E IDOSOS",
        "lotacao_real": 8,
        "lotacao_paradigma": 9,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-547": {
        "comarca": "São José dos Pinhais",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 20,
        "lotacao_paradigma": 20,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-548": {
        "comarca": "São José dos Pinhais",
        "unidade": "VARA DA FAZENDA PÚBLICA",
        "lotacao_real": 9,
        "lotacao_paradigma": 7,
        "diferenca": 2,
        "status": "SUPERAVITÁRIA",
    },
    "A2-549": {
        "comarca": "São José dos Pinhais",
        "unidade": "VARA DA INFÂNCIA E JUVENTUDE",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-550": {
        "comarca": "São José dos Pinhais",
        "unidade": "VARA DE FAMÍLIA E SUCESSÕES, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 7,
        "lotacao_paradigma": 7,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-551": {
        "comarca": "São José dos Pinhais",
        "unidade": "VARA DESCENTRALIZADA DO AFONSO PENA",
        "lotacao_real": 6,
        "lotacao_paradigma": 7,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-552": {
        "comarca": "São João",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 3,
        "lotacao_paradigma": 6,
        "diferenca": -3,
        "status": "DEFICITÁRIA",
    },
    "A2-553": {
        "comarca": "São João",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 4,
        "lotacao_paradigma": 4,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-554": {
        "comarca": "São João do Ivaí",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-555": {
        "comarca": "São João do Lvaí",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-556": {
        "comarca": "São João do Triunfo",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-557": {
        "comarca": "São João do Triunfo",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-558": {
        "comarca": "São Mateus do Sul",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-559": {
        "comarca": "São Mateus do Sul",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-560": {
        "comarca": "São Miguel do Iguaçu",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 4,
        "lotacao_paradigma": 4,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-561": {
        "comarca": "São Miguel do Iguaçu",
        "unidade": "VARA CRIMINAL, FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE E JUIZADO ESPECIAL CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-562": {
        "comarca": "Teixeira Soares",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-563": {
        "comarca": "Teixeira Soares",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-564": {
        "comarca": "Telêmaco Borba",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-565": {
        "comarca": "Telêmaco Borba",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 6,
        "lotacao_paradigma": 6,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-566": {
        "comarca": "Telêmaco Borba",
        "unidade": "VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 6,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-567": {
        "comarca": "Telêmaco Borba",
        "unidade": "VARA CÍVEL E DA FAZENDA PÚBLICA",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-568": {
        "comarca": "Telêmaco Borba",
        "unidade": "VARA DE FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 3,
        "lotacao_paradigma": 5,
        "diferenca": -2,
        "status": "DEFICITÁRIA",
    },
    "A2-569": {
        "comarca": "Terra Boa",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 8,
        "lotacao_paradigma": 7,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-570": {
        "comarca": "Terra Boa",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 4,
        "lotacao_paradigma": 4,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-571": {
        "comarca": "Terra Rica",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-572": {
        "comarca": "Terra Rica",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-573": {
        "comarca": "Terra Roxa",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-574": {
        "comarca": "Terra Roxa",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-575": {
        "comarca": "Tibagi",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-576": {
        "comarca": "Tibagi",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-577": {
        "comarca": "Toledo",
        "unidade": "1ª VARA CRIMINAL",
        "lotacao_real": 6,
        "lotacao_paradigma": 5,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-578": {
        "comarca": "Toledo",
        "unidade": "2ª VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-579": {
        "comarca": "Toledo",
        "unidade": "3ª VARA CÍVEL E DA FAZENDA PÚBLICA",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-580": {
        "comarca": "Toledo",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 6,
        "lotacao_paradigma": 6,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-581": {
        "comarca": "Toledo",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 10,
        "lotacao_paradigma": 10,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-582": {
        "comarca": "Toledo",
        "unidade": "VARA DA INFÂNCIA E JUVENTUDE, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 6,
        "lotacao_paradigma": 5,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-583": {
        "comarca": "Toledo",
        "unidade": "VARA DE FAMÍLIA E SUCESSÕES",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-584": {
        "comarca": "Tomazina",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-585": {
        "comarca": "Tomazina",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-586": {
        "comarca": "Ubiratã",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-587": {
        "comarca": "Ubiratã",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-588": {
        "comarca": "Umuarama",
        "unidade": "1ª VARA CRIMINAL",
        "lotacao_real": 3,
        "lotacao_paradigma": 5,
        "diferenca": -2,
        "status": "DEFICITÁRIA",
    },
    "A2-589": {
        "comarca": "Umuarama",
        "unidade": "1ª VARA CÍVEL E DA FAZENDA PÚBLICA",
        "lotacao_real": 7,
        "lotacao_paradigma": 5,
        "diferenca": 2,
        "status": "SUPERAVITÁRIA",
    },
    "A2-590": {
        "comarca": "Umuarama",
        "unidade": "2ª VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-591": {
        "comarca": "Umuarama",
        "unidade": "3ª VARA CÍVEL E DA FAZENDA PÚBLICA",
        "lotacao_real": 6,
        "lotacao_paradigma": 5,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-592": {
        "comarca": "Umuarama",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-593": {
        "comarca": "Umuarama",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 15,
        "lotacao_paradigma": 15,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-594": {
        "comarca": "Umuarama",
        "unidade": "VARA DE FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 6,
        "lotacao_paradigma": 7,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-595": {
        "comarca": "União da Vitória",
        "unidade": "1ª VARA CRIMINAL",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-596": {
        "comarca": "União da Vitória",
        "unidade": "2ª VARA CRIMINAL",
        "lotacao_real": 4,
        "lotacao_paradigma": 6,
        "diferenca": -2,
        "status": "DEFICITÁRIA",
    },
    "A2-597": {
        "comarca": "União da Vitória",
        "unidade": "2ª VARA CÍVEL E DA FAZENDA PÚBLICA",
        "lotacao_real": 4,
        "lotacao_paradigma": 5,
        "diferenca": -1,
        "status": "DEFICITÁRIA",
    },
    "A2-598": {
        "comarca": "União da Vitória",
        "unidade": "JUIZADO ESPECIAL CÍVEL, CRIMINAL E DA FAZENDA PÚBLICA",
        "lotacao_real": 6,
        "lotacao_paradigma": 5,
        "diferenca": 1,
        "status": "SUPERAVITÁRIA",
    },
    "A2-599": {
        "comarca": "União da Vitória",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 14,
        "lotacao_paradigma": 14,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-600": {
        "comarca": "União da Vitória",
        "unidade": "VARA DE FAMÍLIA E SUCESSÕES, INFÂNCIA E JUVENTUDE, ACIDENTES DO TRABALHO, REGISTROS PÚBLICOS E CORREGEDORIA DO FORO EXTRAJUDICIAL",
        "lotacao_real": 7,
        "lotacao_paradigma": 7,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-601": {
        "comarca": "Uraí",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 7,
        "lotacao_paradigma": 5,
        "diferenca": 2,
        "status": "SUPERAVITÁRIA",
    },
    "A2-602": {
        "comarca": "Uraí",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-603": {
        "comarca": "Wenceslau Braz",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-604": {
        "comarca": "Wenceslau Braz",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-605": {
        "comarca": "Xambrê",
        "unidade": "JUÍZO ÚNICO",
        "lotacao_real": 5,
        "lotacao_paradigma": 5,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
    "A2-606": {
        "comarca": "Xambrê",
        "unidade": "SECRETARIA DA DIREÇÃO DO FÓRUM",
        "lotacao_real": 3,
        "lotacao_paradigma": 3,
        "diferenca": 0,
        "status": "EQUILIBRADA",
    },
}
